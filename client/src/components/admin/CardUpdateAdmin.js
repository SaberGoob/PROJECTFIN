...
const CardUpdate = ({ship,show,setShow,setUpdate,ping,setPing}) => {
  ...
  return (
    ...
    <p1>Company:<input
     ...
     <p1>Etat:
      <select  onChange={(e) => setNewShip({ ...newShip, etat: e.target.value })} defaultValue={ship?.etat}>
    ...
    <p>Date Pick Up:<input
      onChange={(e) => setNewShip({ ...newShip, pickup: e.target.value })} defaultValue={ship?.pickup}
     /> </p>
